...
class IoControl extends Module {
  ...
  val dcache_read_other = !dcache_read_base && !dcache_read_ext && !dcache_read_uart &&
    !dcache_read_uart_state && dcache_read_req_valid
  ...
  // ready 信号：当没有正在处理的请求且状态机空闲时
  io.icache_read_req.ready := !icache_req_valid && (base_state === sIDLE) &&
    (ext_state === sIDLE) && (other_state === oIDLE)
  ...
  io.dcache_read_req.ready := !dcache_read_req_valid && (base_state === sIDLE) &&
    (ext_state === sIDLE) && (other_state === oIDLE) &&
    (uart_state === uIDLE)
  ...
  io.dcache_write_req.ready := !dcache_write_req_valid && (base_state === sIDLE) &&
    (ext_state === sIDLE) && (other_state === oIDLE) &&
    (uart_state === uIDLE) && !io.txd.uart_busy

  // 捕获请求信息
  when(io.icache_read_req.valid && io.icache_read_req.ready) {
    icache_req_addr  := io.icache_read_req.bits.addr
    icache_req_valid := true.B
  }

  when(io.dcache_read_req.valid && io.dcache_read_req.ready) {
    dcache_read_req_addr  := io.dcache_read_req.bits.addr
    dcache_read_req_valid := true.B
  }

  when(io.dcache_write_req.valid && io.dcache_write_req.ready) {
  ...
  switch(base_state) {
    ...
    is(iREAD) {
      when(base_clock_counter === (FETCH_WIDTH - 1).U) {
  ...
  switch(ext_state) {
    ...
    is(iREAD) {
      when(ext_clock_counter === (FETCH_WIDTH - 1).U) {
  ...
  switch(uart_state) {
    ...
    is(uWRITE) {
      ...
      dcache_write_req_valid := false.B // 清除写请求
      uart_state             := uIDLE
    }
    is(uREAD) {
      dcache_data_valid     := false.B
      dcache_read_req_valid := false.B // 清除读请求
      uart_state            := uIDLE
    }
  }
