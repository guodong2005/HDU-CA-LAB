...
class Core extends Module {
  val io = IO(new Bundle {
    ...
    val debug_rd = Output(UInt(5.W)); val debug_wdata = Output(UInt(32.W)); val debug_wen = Output(Bool())
  ...
  val draining = RegInit(false.B); val halted = RegInit(false.B)
  ...
  val loadUse = ifid.valid && idex.valid && idex.control.memRead && idex.rd =/= 0.U && (idex.rd===idRs1 || idex.rd===idRs2)
  ...
  when(idex.control.memRead || idex.control.memWrite){exResult:=exRs1+idex.immediate}.elsewhen(idex.control.writeUpperImmediate){exResult:=idex.immediate}.elsewhen(idex.control.writePcRelative){exResult:=idex.pc+idex.immediate}.elsewhen(idex.control.jump){exResult:=idex.pc+4.U}.elsewhen(idex.control.aluOp===5.U){switch(idex.instr(14,12)){is(0.U){exResult:=(exRs1*exRs2)(31,0)};is(4.U){exResult:=Mux(exRs2===0.U,"hffffffff".U,(exRs1.asSInt/exRs2.asSInt).asUInt)};is(5.U){exResult:=Mux(exRs2===0.U,"hffffffff".U,exRs1/exRs2)};is(6.U){exResult:=Mux(exRs2===0.U,exRs1,(exRs1.asSInt%exRs2.asSInt).asUInt)};is(7.U){exResult:=Mux(exRs2===0.U,exRs1,exRs1%exRs2)}}}.otherwise{switch(idex.instr(14,12)){is(0.U){exResult:=Mux(idex.instr(6,0)==="b0110011".U && idex.instr(30),exRs1-exOp2,exRs1+exOp2)};is(1.U){exResult:=exRs1<<exOp2(4,0)};is(2.U){exResult:=(exRs1.asSInt<exOp2.asSInt).asUInt};is(3.U){exResult:=(exRs1<exOp2).asUInt};is(4.U){exResult:=exRs1^exOp2};is(5.U){exResult:=Mux(idex.instr(30),(exRs1.asSInt>>exOp2(4,0)).asUInt,exRs1>>exOp2(4,0))};is(6.U){exResult:=exRs1|exOp2};is(7.U){exResult:=exRs1&exOp2}}}
  ...
  io.axi.ar.valid := owner===mReadReq || (owner===fReq && !launchMem && !taken); io.axi.ar.bits:=0.U.asTypeOf(new AxiAR); io.axi.ar.bits.addr:=Mux(owner===mReadReq,memAddr&"hfffffffc".U,pc); io.axi.ar.bits.id:=0.U; io.axi.ar.bits.size:=2.U
  ...
  io.debug_commit:=false.B; io.debug_pc:=0.U; io.debug_instr:=0.U; io.debug_rd:=0.U; io.debug_wdata:=0.U; io.debug_wen:=false.B; io.halted:=halted
  ...
  when(fetchResponse){when(!draining){ifid.valid:=true.B;ifid.pc:=pc;ifid.instr:=io.axi.r.bits.data;pc:=pc+4.U};owner:=fReq}
  when(owner===mReadReq&&io.axi.ar.fire){owner:=mReadWait}
  when(owner===mReadWait&&io.axi.r.fire){val word=io.axi.r.bits.data;val byte=MuxLookup(memAddr(1,0),word(7,0))(Seq(1.U->word(15,8),2.U->word(23,16),3.U->word(31,24)));val half=Mux(memAddr(1),word(31,16),word(15,0));val v=Mux(memSize===0.U,Cat(Fill(24,byte(7)&&memSigned),byte),Mux(memSize===1.U,Cat(Fill(16,half(15)&&memSigned),half),word));memwb.valid:=true.B;memwb.pc:=memPc;memwb.instr:=memInstr;memwb.rd:=memRd;memwb.loadData:=v;memwb.control:=0.U.asTypeOf(new ScalarControl);memwb.control.regWrite:=memRd=/=0.U;memwb.control.memToReg:=true.B;exmem.valid:=false.B;owner:=fReq}
  when(owner===mAw&&io.axi.aw.fire){owner:=mW}; when(owner===mW&&io.axi.w.fire){owner:=mB}; when(owner===mB&&io.axi.b.fire){memwb.valid:=true.B;memwb.pc:=memPc;memwb.instr:=memInstr;memwb.control:=0.U.asTypeOf(new ScalarControl);exmem.valid:=false.B;owner:=fReq}
