    ...
    if (((top->clock && !dual_issue) || dual_issue) &&
      ...
      if ((top->debug_pc != cemu_rvcore.debug_pc ||
        ...
        printf("reference: PC = 0x%016lx, wb_rf_wnum = 0x%02lx, wb_rf_wdata = "
               "0x%016lx\n",
               cemu_rvcore.debug_pc, cemu_rvcore.debug_reg_num,
               cemu_rvcore.debug_reg_wdata);
...
void make_cpu_trace(Vtop *top, nscscc_sram_ref &mmio_ref,
                    const char *riscv_test_path) {
  ...
  while (!Verilated::gotFinish() && sim_time > 0 && running) {
    ...
    if (((top->clock && !dual_issue) || dual_issue) &&
      ...
      fprintf(trace_file, "1 %016lx %02lx %016lx\n", cemu_rvcore.debug_pc,
              cemu_rvcore.debug_reg_num, cemu_rvcore.debug_reg_wdata);
    }
    if (trace_on) {
      fst.dump(ticks);
      sim_time--;
