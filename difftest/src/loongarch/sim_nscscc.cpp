...
void func_run(Vmycpu_top *top, axi4_ref<32, 32, 4> &mmio_ref) {
  ...
  while (!Verilated::gotFinish() && sim_time > 0 && running) {
    ...
    if (top->aclk)
      running = confreg.do_trace(top->debug_wb_pc, top->debug_wb_rf_we,
                                 top->debug_wb_rf_wnum, top->debug_wb_rf_wdata);
...
void perf_run(Vmycpu_top *top, axi4_ref<32, 32, 4> &mmio_ref,
              int test_start = 1, int test_end = 10) {
  axi4<32, 32, 4> mmio_sigs;
  axi4_ref<32, 32, 4> mmio_sigs_ref(mmio_sigs);
...
void cemu_perf_diff(Vmycpu_top *top, axi4_ref<32, 32, 4> &mmio_ref,
                    int test_start = 1, int test_end = 10) {
  uint32_t last_pc = 0;
  ...
  for (int test = test_start; test <= test_end && running; test++) {
    ...
    while (!Verilated::gotFinish() && sim_time > 0 && running && !test_end) {
      ...
      // if (top->aclk && top->debug_wb_rf_we && top->debug_wb_pc !=
      // last_commit_pc)
      {
        do {
          cemu_la32r.step();
        ...
        if (cemu_la32r.debug_wb_pc != top->debug_wb_pc ||
            ...
            (cemu_la32r.debug_wb_wdata != top->debug_wb_rf_wdata &&
             !cemu_la32r.debug_wb_is_timer)) {
          printf("-------------------------------------------------------------"
                 "-------------at hex-\n");
          printf("\e[33mlast     : PC = %08x, wb_rf_wnum = %02x, wb_rf_wdata = "
                 "%08x\n",
                 last_pc, last_wnum, last_wdata);
          printf(
              "reference: PC = %08x, wb_rf_wnum = %02x, wb_rf_wdata = %08x\n",
              cemu_la32r.debug_wb_pc, cemu_la32r.debug_wb_wnum,
              cemu_la32r.debug_wb_wdata);
          printf("mycpu    : PC = %08x, wb_rf_wnum = %02x, wb_rf_wdata = "
                 "%08x\e[0m\n",
                 top->debug_wb_pc, top->debug_wb_rf_wnum,
                 top->debug_wb_rf_wdata);
          printf("-------------------------------------------------------------"
                 "-------------at hex-\n");
          running = false;
          // FILE *fp =
          // fopen("/mnt/f/CPU/lab_Loongarch/mycpu_env/func/obj/test.s", "r");
          // char inst[100];
          ...
          if (cemu_la32r.debug_wb_is_timer) {
            cemu_la32r.set_GPR(cemu_la32r.debug_wb_wnum,
                               top->debug_wb_rf_wdata);
          }
          inst_count++;
        }
        ...
        // {
        //     printf("Error!\n");
        //     printf("reference: PC = 0x%08x, wb_rf_wnum = 0x%02x, wb_rf_wdata
        //     = 0x%08x\n", cemu_la32r.debug_wb_pc, cemu_la32r.debug_wb_wnum,
        //     cemu_la32r.debug_wb_wdata); printf("mycpu    : PC = 0x%08x,
        //     wb_rf_wnum = 0x%02x, wb_rf_wdata = 0x%08x\n", top->debug_wb_pc,
        //     top->debug_wb_rf_wnum, top->debug_wb_rf_wdata); running = false;
        // }
        // else
        // {
        //     if (cemu_la32r.debug_wb_is_timer)
...
void ucore_run(Vmycpu_top *top, axi4_ref<32, 32, 4> &mmio_ref) {
  ...
  // sys_mem at 0x0
  // mmio_mem sys_mem(128*1024*1024,
  // "../ucore-thumips/obj/ucore-kernel-initrd.bin");
  mmio_mem sys_mem(128 * 1024 * 1024,
                   "/mnt/f/CPU/lab_Loongarch/mycpu_env/func/obj/main.bin");
