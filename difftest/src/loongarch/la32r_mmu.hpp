...
public:
    ...
    la32r_exccode va_if(uint32_t addr, unsigned char *buffer, la32r_plv cur_plv, bool map, uint32_t asid)
    {
        if (addr % 4 != 0)
        {
    ...
    la32r_exccode va_read(uint32_t addr, uint32_t size, unsigned char *buffer, la32r_plv cur_plv, bool map, uint32_t asid)
    {
        if (addr % size != 0)
        {
        ...
        if (!map)
        {
            ...
            // printf("read:  Not Mapped, addr = %08x, rdata = %08x\n", addr, *(uint32_t *)buffer);
            return std::make_pair(OK, 0);
        }
        auto dmwe = dmw_match(addr, cur_plv);
        if (dmwe != nullptr)
        {
            ...
            // printf("read:  DMW_hit, addr = %08x, rdata = %08x\n", (dmwe->pseg << 29) | (addr & 0x1fffffffu), *(uint32_t *)buffer);
            return std::make_pair(OK, 0);
        }
        // printf("read:  others\n");
        if (cur_plv == plv3 && addr >= 0x80000000u)
    ...
    la32r_exccode va_write(uint32_t addr, uint32_t size, unsigned char *buffer, la32r_plv cur_plv, bool map, uint32_t asid)
    {
        if (addr % size != 0)
        {
            return std::make_pair(ALE, 0);
        ...
        if (!map)
        {
            ...
            // printf("write: Not Mapped, addr = %08x, wdata = %08x\n", addr, *(uint32_t *)buffer);
            return std::make_pair(OK, 0);
        }
        auto dmwe = dmw_match(addr, cur_plv);
        if (dmwe != nullptr)
        {
            ...
            // printf("write: DMW_hit, addr = %08x, wdata = %08x\n", (dmwe->pseg << 29) | (addr & 0x1fffffffu), *(uint32_t *)buffer);
            return std::make_pair(OK, 0);
        }
        // printf("write: others\n");
        if (cur_plv == plv3 && addr >= 0x80000000u)
...
private:
    ...
    la32r_tlb *tlb_match(uint32_t va, uint32_t asid)
    {
        for (int i = 0; i < nr_tlb_entry; i++)
        {
            if (tlb[i].e && (asid == tlb[i].asid || tlb[i].g) && (tlb[i].vppn >> (tlb[i].ps - 12)) == (va >> (tlb[i].ps + 1)))
            {
                return &tlb[i];
            }
        }
