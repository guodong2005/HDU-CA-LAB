...
public:
    ...
    void set_GPR(uint8_t GPR_index, int64_t value)
    {
        ...
        if (GPR_index)
            GPR[GPR_index] = value;
        debug_reg_num = GPR_index;
        debug_reg_wdata = value;
    }
    uint64_t getPC()
    {
        return pc;
...
private:
    ...
    void exec(bool meip, bool msip, bool mtip, bool seip)
    {
        ...
        bool ri = false; // illegal instruction
        bool new_pc = false;
        bool is_rvc = false;
        ...
        if (!is_rvc)
        {
            ...
            case OPCODE_OPIMM:
            {
                ...
                case FUNCT3_SRL_SRA:
                    ...
                    if (fun6 != FUNCT6_NORMAL && fun6 != FUNCT6_SRA)
                        ri = true;
                    break;
                default:
                    assert(false);
            ...
            case OPCODE_OPIMM32:
            {
                ...
                case FUNCT3_SLL:
                    if (imm >= 64)
                        ri = true;
                    op = ALU_SLL;
                    if (fun7 != FUNCT7_NORMAL)
                        ri = true;
                    break;
                case FUNCT3_SRL_SRA:
                    imm = imm & ((1 << 6) - 1);
                    op = ((fun7 == FUNCT7_SUB_SRA) ? ALU_SRA : ALU_SRL);
                    if (fun7 != FUNCT7_NORMAL && fun7 != FUNCT7_SUB_SRA)
                        ri = true;
                    break;
                default:
                    ri = true;
                    ...
                    else
                    {
                        if (inst->r_type.funct3 == 0b011)
                        {
                            ...
                            if (exc == exc_custom_ok)
                                ...
                                priv.raise_trap(csr_cause_def(exc), GPR[inst->r_type.rs1]);
                        }
                        else
                        {
                            int32_t result;
            ...
            case OPCODE_SYSTEM:
            {
                ...
                case FUNCT3_CSRRW:
                {
                    ...
                    uint64_t csr_result;
                    if (!ri)
                        ri = !priv.csr_read(csr_index, csr_result);
                    if (!ri)
        ...
        else
        {
            ...
            else
            {
                ...
                case OPCODE_C_ADDI:
                {
                    ...
                    if (imm >> 5)
                        imm |= 0xffffffffffffffc0u; // sign extend[5]
                    uint8_t rd = binary_concat(cur_instr, 11, 7, 0);
                    if (imm)
                        set_GPR(rd, alu_exec(GPR[rd], imm, ALU_ADD)); // nzimm
                    break;
                }
                case OPCODE_C_ADDIW:
                {
                ...
                case OPCODE_C_ADDI16SPN_LUI:
                {
                    ...
                    { // ADDI16SPN
                        ...
                        if (imm >> 9)
                            imm |= 0xfffffffffffffc00u; // sign extend[9]
                    ...
                    { // LUI
                        ...
                        if (imm >> 17)
                            imm |= 0xfffffffffffc0000u; // sign extend[17]
                        if (imm)
                            set_GPR(rd, imm); // nzimm
                    }
                    break;
                }
                case OPCODE_C_ALU:
                        ...
                        else
                        {
                            ...
                            case FUNCT2_OR:
                            {
                                if (instr_12)
                                    ...
                                    set_GPR(rs1, alu_exec(GPR[rs1], GPR[rs2], ALU_OR));
                                break;
                            }
                            case FUNCT2_AND:
                            {
                                if (instr_12)
                                    ...
                                    set_GPR(rs1, alu_exec(GPR[rs1], GPR[rs2], ALU_AND));
                                break;
                            }
                            }
                        }
                ...
                case OPCODE_C_J:
                {
                    ...
                    if (imm >> 11)
                        imm |= 0xfffffffffffff000u; // sign extend [11]
                    uint64_t npc = pc + imm;
                    pc = npc;
                    new_pc = true;
                    break;
                ...
                case OPCODE_C_BEQZ:
                {
                    ...
                    if (imm >> 8)
                        imm |= 0xfffffffffffffe00u; // sign extend [8]
                    uint8_t rs1 = 8 + binary_concat(cur_instr, 9, 7, 0);
                    if (GPR[rs1] == 0)
                    {
                        pc = pc + imm;
                ...
                case OPCODE_C_SLLI:
                {
                    ...
                    if (imm)
                        set_GPR(rs1, alu_exec(GPR[rs1], imm, ALU_SLL)); // nzimm
                    break;
                }
                case OPCODE_C_LWSP:
                {
