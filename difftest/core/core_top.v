...
module FetchUnit(
  ...
  reg           canStart_REG;
  reg           respValid;
  reg  [4095:0] respData_data;
  reg  [31:0]   respData_addr;
  reg           gotResponse;
  wire          canSendReq = canStart_REG & ~reset & (~pendingReq | gotResponse);
  wire [31:0]   nextReqPC =
    io_signal_branchControl_branch
      ? io_signal_branchControl_target
      : pendingReq & gotResponse ? prefetchPC : pc;
  wire          _GEN = respValid & pendingReq & respData_addr == reqPC;
  wire [31:0]   inst =
    reqPC[8:2] == 7'h7
      ? respData_data[255:224]
      : reqPC[8:2] == 7'h6
          ? respData_data[223:192]
          : reqPC[8:2] == 7'h5
              ? respData_data[191:160]
              : reqPC[8:2] == 7'h4
                  ? respData_data[159:128]
                  : reqPC[8:2] == 7'h3
                      ? respData_data[127:96]
                      : reqPC[8:2] == 7'h2
                          ? respData_data[95:64]
                          : reqPC[8:2] == 7'h1
                              ? respData_data[63:32]
                              : reqPC[8:2] == 7'h0 ? respData_data[31:0] : 32'h0;
  wire          isExpectedPC = reqPC == pc;
  wire          _GEN_0 = io_signal_fetchUnitSignal_allow_to_go & ~ifid_reg_valid;
  wire          _GEN_1 = _GEN & isExpectedPC;
  wire          _GEN_2 = _GEN & isExpectedPC & _GEN_0;
  ...
  always @(posedge clock) begin
    ...
    if (reset) begin
      ...
      automatic logic _GEN_5;
      _GEN_5 = _GEN_1 & ~_GEN_0 | ifid_reg_valid;
      if (&missCounter)
        pc <= reqPC;
      ...
      if (io_signal_branchControl_branch) begin
        ...
        if (_GEN_4 & ~io_signal_branchControl_branch)
      ...
      gotResponse <= io_icache_resp_valid & pendingReq;
      if ((&missCounter) | ~(pendingReq & ~gotResponse))
        missCounter <= 8'h0;
      else
        missCounter <= missCounter + 8'h1;
    end
    if (_GEN_4)
      reqPC <= nextReqPC;
    canStart_REG <= ~reset;
    respValid <= io_icache_resp_valid;
    respData_data <= io_icache_resp_bits_data;
    respData_addr <= io_icache_resp_bits_addr;
  end // always @(posedge)
  `ifdef ENABLE_INITIAL_REG_
    `ifdef FIRRTL_BEFORE_INITIAL
      `FIRRTL_BEFORE_INITIAL
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        pc = _RANDOM[8'h0];
        prefetchPC = _RANDOM[8'h1];
        reqPC = _RANDOM[8'h2];
        pendingReq = _RANDOM[8'h3][0];
        ifid_reg_inst = {_RANDOM[8'h3][31:1], _RANDOM[8'h4][0]};
        ifid_reg_valid = _RANDOM[8'h4][1];
        ifid_reg_pc = {_RANDOM[8'h4][31:2], _RANDOM[8'h5][1:0]};
        canStart_REG = _RANDOM[8'h5][2];
        respValid = _RANDOM[8'h5][3];
        respData_data =
          {_RANDOM[8'h5][31:4],
           _RANDOM[8'h6],
           _RANDOM[8'h7],
           ...
           _RANDOM[8'h84],
           _RANDOM[8'h85][3:0]};
        respData_addr = {_RANDOM[8'h85][31:4], _RANDOM[8'h86][3:0]};
        gotResponse = _RANDOM[8'h86][4];
        missCounter = _RANDOM[8'h86][12:5];
      `endif // RANDOMIZE_REG_INIT
    end // initial
    `ifdef FIRRTL_AFTER_INITIAL
      `FIRRTL_AFTER_INITIAL
    `endif // FIRRTL_AFTER_INITIAL
  `endif // ENABLE_INITIAL_REG_
  assign io_decodeStage_data_inst = _GEN_3 ? ifid_reg_inst : _GEN_2 ? inst : 32'h0;
  assign io_decodeStage_data_valid = _GEN_3 ? ifid_reg_valid : _GEN_1 & _GEN_0;
  assign io_decodeStage_data_pc = _GEN_3 ? ifid_reg_pc : _GEN_2 ? reqPC : 32'h0;
